...
export function Profile() {
  ...
  const handleExportUserData = async () => {
    try {
      ...
      // Download JSON
      downloadFile(jsonBlob, `financehub-dados-completos-${new Date().toISOString().split('T')[0]}.json`)
      
      // Download CSV (após pequeno delay)
      setTimeout(() => {
        downloadFile(csvBlob, `financehub-transacoes-${new Date().toISOString().split('T')[0]}.csv`)
      }, 1000)
