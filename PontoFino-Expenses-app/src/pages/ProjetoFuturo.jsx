  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-blue-900 to-[#0096fd] relative overflow-hidden">
      ...
      <main className="container mx-auto px-4 py-16 flex-1 w-full relative z-10 flex flex-col items-center">
        <h1 className="text-4xl md:text-5xl font-extrabold text-center mb-8 text-primary animate-slide-up drop-shadow-lg">Projetos Futuros</h1>
        <ul className="flex flex-col gap-8 text-gray-200 text-lg max-w-2xl mx-auto animate-fade-in w-full" style={{animationDelay: '0.3s', animationFillMode: 'both'}}>
          {projects.map((proj, idx) => (
            <li key={proj.text} className="flex items-center gap-5 bg-background/60 rounded-2xl px-8 py-6 shadow-xl border border-white/10 hover:scale-105 transition-transform duration-300 animate-slide-up" style={{animationDelay: `${0.4 + idx * 0.07}s`, animationFillMode: 'both'}}>
              <span>{proj.icon}</span>
              <span>{proj.text}</span>
            </li>
          ))}
        ...
        <div className="mt-16 text-center animate-fade-in" style={{animationDelay: '0.7s', animationFillMode: 'both'}}>
          <h2 className="text-2xl font-bold mb-2 text-primary animate-slide-up">E muito mais por vir!</h2>
          <p className="text-gray-200">Estamos sempre inovando para entregar a melhor experiência financeira para você. Fique ligado nas novidades!</p>
        </div>
      </main>
      <Footer />
    </div>
