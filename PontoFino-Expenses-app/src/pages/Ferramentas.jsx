...
const tools = [
  {
    ...
    icon: (
      <svg className="w-12 h-12 text-blue-400 mb-3 " fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M3 10h18M9 16h6M12 6v10" /></svg>
    ),
  ...
  },
  {
    ...
    icon: (
      <svg className="w-12 h-12 text-green-400 mb-3 animate-pulse" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 19V5m7 7H5" /></svg>
    ),
  ...
  },
  {
    ...
    icon: (
      <svg className="w-12 h-12 text-yellow-400 mb-3 animate-spin-slow" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>
    ),
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-blue-900 to-[#0096fd] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-12 min-h-[60vh] flex-1 w-full relative z-10">
        <h1 className="text-4xl md:text-5xl font-extrabold text-center mb-12 text-primary animate-slide-up drop-shadow-lg">Ferramentas da Plataforma</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {tools.map((tool, idx) => (
            <div
              key={tool.title}
              className={`bg-background rounded-2xl p-10 shadow-2xl flex flex-col items-center border border-white/10 relative overflow-hidden animate-slide-up transition-all duration-300 group ${tool.available ? 'hover:scale-105 hover:shadow-blue-400/30' : 'opacity-70 grayscale'}`}
            ...
            >
              ...
              <h2 className="text-2xl font-bold mb-2 group-hover:text-blue-300 transition-colors duration-200 text-center">{tool.title}</h2>
              <p className="text-gray-400 mb-6 text-center">{tool.desc}</p>
              {tool.action}
              {!tool.available && (
        ...
        <div className="mt-16 text-center animate-fade-in" style={{animationDelay: '0.4s', animationFillMode: 'both'}}>
          <h3 className="text-xl font-bold mb-2 text-primary animate-slide-up">Mais ferramentas em breve!</h3>
