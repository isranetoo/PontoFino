	...
	return (
		<div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-blue-900 to-[#0096fd] relative overflow-hidden">
			...
			<div className="absolute top-0 left-0 w-full h-full pointer-events-none z-0">
				<svg
					className="absolute left-10 top-10 animate-float-slow opacity-20"
					width="120"
					height="120"
					fill="none"
					viewBox="0 0 120 120"
				...
				<svg
					className="absolute right-10 bottom-10 animate-float-slow opacity-10"
					width="160"
					height="160"
					fill="none"
					viewBox="0 0 160 160"
				...
				<svg
					className="absolute right-1/3 top-1/4 animate-float-slow opacity-10"
					width="100"
					height="100"
					fill="none"
					viewBox="0 0 100 100"
			...
			<div className="container mx-auto px-4 py-12 min-h-[60vh] flex-1 w-full relative z-10">
				<h1 className="text-4xl md:text-5xl font-extrabold text-center mb-4 animate-slide-up drop-shadow-lg" style={{ color: 'white' }}>
					Ferramentas da Plataforma
				</h1>
				<p className="text-center text-lg text-blue-100 mb-10 animate-fade-in" style={{ animationDelay: '0.2s' }}>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-10">
					{tools.map((tool, idx) => (
						...
						>
							{/* Decorative blob */}
							<svg
								className="absolute -top-8 -right-8 w-24 h-24 opacity-10 group-hover:opacity-20 transition-opacity duration-300"
								fill="#00b6fc"
								viewBox="0 0 100 100"
							>
							...
							<h2 className="text-lg sm:text-xl md:text-2xl font-bold mb-1 sm:mb-2 group-hover:text-blue-300 transition-colors duration-200 text-center">
								{tool.title}
							</h2>
							<p className="text-gray-400 mb-2 sm:mb-4 text-center text-sm sm:text-base">
								{tool.desc}
							</p>
							<ul className="mb-4 text-xs sm:text-sm text-blue-200 list-disc list-inside text-left w-full max-w-xs mx-auto">
							...
							{tool.action}
							{!tool.available && (
								<span className="absolute top-4 right-4 bg-yellow-400 text-yellow-900 text-xs font-bold px-3 py-1 rounded-full animate-pulse shadow">
									Em breve
								</span>
							)}
						</div>
				...
				<div className="mt-16 text-center animate-fade-in" style={{ animationDelay: '0.4s', animationFillMode: 'both' }}>
					<h3 className="text-2xl font-bold mb-2 animate-slide-up" style={{ color: 'white' }}>
						Por que usar nossas ferramentas?
					</h3>
					<div className="flex flex-col md:flex-row justify-center gap-6 mt-4">
						<div className="bg-white/10 rounded-xl p-6 flex-1 min-w-[220px] shadow-lg">
				...
				<div className="mt-20 animate-fade-in" style={{ animationDelay: '0.6s', animationFillMode: 'both' }}>
					...
					<div className="flex flex-col md:flex-row gap-6 justify-center items-center">
						{testimonials.map((t, i) => (
							<div key={i} className="bg-white/10 rounded-2xl p-6 shadow-lg flex flex-col items-center max-w-xs">
								<img
									src={t.avatar}
									alt={t.name}
									className="w-14 h-14 rounded-full mb-3 border-2 border-blue-300 shadow"
								/>
								<p className="text-blue-100 italic mb-2">"{t.text}"</p>
								<span className="text-blue-200 font-semibold">{t.name}</span>
							</div>
						))}
					</div>
				</div>
				{/* Call to action */}
				<div className="mt-16 text-center animate-fade-in" style={{ animationDelay: '0.8s', animationFillMode: 'both' }}>
					<h3 className="text-xl font-bold mb-2 animate-slide-up" style={{ color: 'white' }}>
						Tem sugestões ou quer ver uma nova funcionalidade?
					</h3>
					<p className="text-gray-200 animate-fade-in mb-4">
				...
				<div className="mt-10 text-center animate-fade-in" style={{ animationDelay: '1s', animationFillMode: 'both' }}>
					<h3 className="text-lg font-bold mb-2 animate-slide-up" style={{ color: 'white' }}>
